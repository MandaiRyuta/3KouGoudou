...
#include "meshfiled.h"
#include "CModeGame.h"
D3DXMATRIX CBox::view_;
D3DXMATRIX CBox::proj_;
...
CBox * CBox::Create(MATRIX3DMANAGER::Model * model, std::string filename)
{
	CBox* CreateModel = new CBox(0, model, filename);
	CreateModel->Init();
	return CreateModel;
}

void CBox::SetMtxView(D3DXMATRIX view)
{
	view_ = view;
}

void CBox::SetMtxProj(D3DXMATRIX proj)
{
	proj_ = proj;
}
