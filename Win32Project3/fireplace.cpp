...
#include "meshfiled.h"
#include "CModeGame.h"
D3DXMATRIX CFirePlace::view_;
D3DXMATRIX CFirePlace::proj_;

void CFirePlace::Init()
{
	...
	////シェーダーを読み込む
	if (FAILED(D3DXCreateEffectFromFile(pDevice, "Min.fx", NULL, NULL, 0, NULL, &effect_, NULL)))
	{
	...
	spherecolisioninfo_.modelpos = this->model_->position_;
	spherecolisioninfo_.r = 10.0f;
	D3DXMATRIX scale;
...
void CFirePlace::Update()
{
}

void CFirePlace::Draw()
{
...
CFirePlace * CFirePlace::Create(MATRIX3DMANAGER::Model * model, std::string filename)
{
	CFirePlace* CreateModel = new CFirePlace(0, model, filename);
	CreateModel->Init();
	return CreateModel;
}

void CFirePlace::SetMtxView(D3DXMATRIX view)
{
	view_ = view;
}

void CFirePlace::SetMtxProj(D3DXMATRIX proj)
{
	proj_ = proj;
}

SphereInfo CFirePlace::Set()
{
	return spherecolisioninfo_;
}

D3DXVECTOR3 CFirePlace::SetPosition()
{
	return this->model_->position_;
}
