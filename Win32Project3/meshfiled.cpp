	...
	*/
	...
	for (int nZ = 0; nZ < nCy; nZ++) {
		for (int nX = 0; nX < nCx; nX++) {
			...
			mpv[nCount].pos = D3DXVECTOR3(fStartX + (this->mesh_info_.size_x * nX), g_FiledHeight[nZ][nX], fStartZ - (this->mesh_info_.size_z * nZ));

			vx = mpv[nZ * nCy + (nX + 1)].pos - mpv[nZ * nCx + (nX - 1)].pos;
			vz = mpv[(nZ + 1) + nCy * nX].pos - mpv[nZ - 1 + nCy * nX].pos;
			...
			mpv[nCount].texcoord = D3DXVECTOR2((this->mesh_info_.size_x * nX) / 1000, (this->mesh_info_.size_z * nZ) / 1000);
	...
	//
	//if (FAILED(hr = D3DXCreateEffectFromFile(pDevice, "meshfiled.fx", NULL, NULL, 0, NULL, &seffect_, &errBuf)))
	//{
	...
	//if (FAILED(D3DXCreateTextureFromFile(pDevice, "resource/texture/Stage.bmp", &pHeightTexture)))
	//{
	//	MessageBox(NULL, "テクスチャの読み込みに失敗しました", NULL, MB_OK);
	//}
	//pHeightTexture->GetLevelDesc(0, &desc);
	//if (FAILED(D3DXCreateTexture(pDevice, desc.Width, desc.Height, 0, 0, D3DFMT_X8R8G8B8, D3DPOOL_MANAGED, &hightmap_)))
	//{
	//	MessageBox(NULL, "テクスチャの読み込みに失敗しました", NULL, MB_OK);
	//}
